...
# .zshrc/.zshenv/.zprofile/.p10k.zsh into $HOME and the zshrc.d/zshenv.d/
# zprofile.d fragment dirs into ~/.config/zsh. Safe to re-run.
set -euo pipefail
...
for f in "$HOME/.zshrc" "$HOME/.zshenv" "$HOME/.zprofile" "$HOME/.p10k.zsh"; do
	if [ -L "$f" ]; then
		rm "$f"
	elif [ -e "$f" ]; then
...
# See ensure_zsh_fragment_dirs in lib/install-common.sh: other packages
# (e.g. tmux/) also place files under
# ~/.config/zsh/{zshrc.d,zshenv.d,zprofile.d}, so these need to be real
# directories before anyone stows into them.
