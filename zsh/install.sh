...
# .zshrc/.zshenv/.p10k.zsh into $HOME and the zshrc.d/zshenv.d fragment dirs
# into ~/.config/zsh. Safe to re-run.
set -euo pipefail
...
# (e.g. tmux/) also place files under ~/.config/zsh/{zshrc.d,zshenv.d}, so
# these need to be real directories before anyone stows into them.
